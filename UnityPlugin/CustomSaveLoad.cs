...
public class CustomSaveLoad {
    ...
    public CustomSaveLoad() {
        ...
        cfgOnlyActiveDrops = Plugin.instance.config.Bind<float>(new ConfigDefinition("Special Effects", "Only Active Drops"), 0f, new ConfigDescription("If above 0, only items sent/received this amount of time after the client is connected will generate notifications. Adjusted for your network conditions, this can filter out large batches of notifications generated by starting inventory or async play. Does not affect starting inventory; use the Ignore Starting Inventory setting instead.", new AcceptableValueRange<float>(0f, 60f)));
        cfgNoStartingInventory = Plugin.instance.config.Bind<bool>(new ConfigDefinition("Special Effects", "Ignore Starting Inventory"), true, new ConfigDescription("If true, starting inventory items will not generate notifications."));
